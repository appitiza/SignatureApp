...
import net.appitiza.android.drawingpad.drawpad.utils.BezierControlPoints;
import net.appitiza.android.drawingpad.drawpad.utils.BezierPath;
import net.appitiza.android.drawingpad.drawpad.utils.SvgGenerator;
import net.appitiza.android.drawingpad.drawpad.utils.Weights;
import net.appitiza.android.drawingpad.drawpad.view.ViewTreeObserverCompat;
import net.appitiza.android.drawingpad.drawpad.view.ViewCompat;
...
public class SignatureView extends View {
    ...
    private final SvgGenerator mSvgBuilder = new SvgGenerator();
    //View state
    private List<Weights> mPoints;
    ...
    // Cache
    private List<Weights> mPointsCache = new ArrayList<>();
    private BezierControlPoints mControlTimedPointsCached = new BezierControlPoints();
    private BezierPath mBezierCached = new BezierPath();
    ...
    private Weights getNewPoint(float x, float y) {
        int mCacheSize = mPointsCache.size();
        Weights timedPoint;
        if (mCacheSize == 0) {
            // Cache is empty, create a new point
            timedPoint = new Weights();
        } else {
    ...
    private void recyclePoint(Weights point) {
        mPointsCache.add(point);
    }

    private void addPoint(Weights newPoint) {
        mPoints.add(newPoint);

        int pointsCount = mPoints.size();
        if (pointsCount > 3) {

            BezierControlPoints tmp = calculateCurveControlPoints(mPoints.get(0), mPoints.get(1), mPoints.get(2));
            Weights c2 = tmp.c2;
            recyclePoint(tmp.c1);
            ...
            BezierPath curve = mBezierCached.set(mPoints.get(1), c2, c3, mPoints.get(2));

            Weights startPoint = curve.startPoint;
            Weights endPoint = curve.endPoint;
            ...
            Weights firstPoint = mPoints.get(0);
            mPoints.add(getNewPoint(firstPoint.x, firstPoint.y));
        }
    }

    private void addBezier(BezierPath curve, float startWidth, float endWidth) {
    ...
    private BezierControlPoints calculateCurveControlPoints(Weights s1, Weights s2, Weights s3) {
        float dx1 = s1.x - s2.x;
        float dy1 = s1.y - s2.y;
        float dx2 = s2.x - s3.x;
        float dy2 = s2.y - s3.y;
